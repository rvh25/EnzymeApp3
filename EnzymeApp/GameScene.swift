...
class GameScene: SKScene {
        ...
        for row in 0..<NumRows {
            for column in 0..<NumColumns {
                ...
                if component.row == 0 && component.column == 0 {
                    ...
                    print("dataConverter: \(dataConverter)")
                    print("horzDelta, vertDelta: \(horzDelta, vertDelta)")
                    let component1 = level.componentAtColumn(swipeFromColumn! + horzDelta, row: swipeFromRow! + vertDelta)
                    let component2 = level.componentAtColumn(swipeFromColumn!, row: swipeFromRow!)
                ...
                else if component.row == NumRows - 1 {
                    let dataConverter = Int(arc4random_uniform(3)+1)
                    if dataConverter == 1 {
                        horzDelta = -1
                    }
                ...
                else if component.column == NumColumns - 1 {
                    let dataConverter = Int(arc4random_uniform(3)+1)
                    if dataConverter == 1 {
                        horzDelta = -1
                    }
    ...
    func trySwapHorizontal2(component1: Component, component2: Component) {
        ...
        //if fromColumn < 0 || fromColumn >= NumColumns { return }
        //if fromRow < 0 || fromRow >= NumRows {return}
        
        let type = ComponentType.Substrate
        //let type2 = ComponentType.Substrate
        //if let toComponent = level.componentAtColumn(toColumn, row: toRow) {
          //  if let fromComponent = level.componentAtColumn(swipeFromColumn!, row: swipeFromRow!) {
       // if let toComponent = level.componentAtColumn(swipeFromColumn!, row: swipeFromRow!) {
          //  if let fromComponent = level.componentAtColumn(fromColumn, row: fromRow) {
